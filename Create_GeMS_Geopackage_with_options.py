...
def create_orientation_lines():
    gpkg.create_feature_class('OrientationLines', srs, fields=get_fields("OrientationLines"), shape_type=GeometryType.multi_linestring)
...
# make sure no \ end up where there should be a /
geopkg_location_clean = ''
for char in geopkg_location:
    if char == '\\':
        char = char.replace('\\', '/')
        geopkg_location_clean += char
    else:
        geopkg_location_clean += char
...
print("Thank you. Now we will add the optional features classes")
time.sleep(1)
os.system('cls')
...
# get optional feature classes
print("Please enter any optional feature classes you would like to add.")
print("If no optional feature classes are needed enter NONE")
print("Otherwise enter feature class name, then press ENTER. When done enter DONE.\n")
print(*list_of_feature_classes, sep='\n')

def full_features():
    create_generic_points()
    create_orientation_points()
    create_orientation_lines()
...
end_optional_features_entry = ''
usr_optional_feature_classes = []

while True:
    optional_feature_class = input("Feature class: ")
    if optional_feature_class.upper() == 'DONE':
        break
    if optional_feature_class.upper() == 'ALL':
        full_features()
        break
    usr_optional_feature_classes.append(optional_feature_class.lower())

for feature in usr_optional_feature_classes:
    feature = feature.lower()


for f in usr_optional_feature_classes:
    if f in list_of_optional_feature_classes:
        if f == 'genericpoints':
            f = create_generic_points()
        elif f == 'genericsamples':
            f = create_generic_samples()
        elif f == 'orientationpoints':
            f = create_orientation_points()
        elif f == 'geochronpoints':
            f = create_geochron_points()
        elif f == 'geochempoints':
            f = create_geochem_points()
        elif f == 'stations':
            f = create_stations_points()
        elif f == 'geologiclines':
            f = create_geologic_lines()
        elif f == 'cartographiclines':
            f = create_cartographic_lines()
        elif f == 'isovaluelines':
            f = create_iso_value_lines()
        elif f == 'mapunitlines':
            f = create_map_unit_lines()
        elif f == 'mapunitpoints':
            f = create_map_unit_points()
        elif f == 'mapunitoverlaypolys':
            f = create_map_unit_overlay_polys()
        elif f == 'mapunitoverlaylines':
           f = create_map_unit_overlay_lines()
        elif f == 'overlaypolys':
            f = create_overlay_polys()
        elif f == 'datasourcepolys':
            f = create_data_source_polys()
        elif f == 'repurposedsymbolstable':
            f = create_repurposed_symbols_table()
        elif f == 'cmumapunitpolys':
            f = create_cmu_map_unit_polys()
        elif f == 'cmulines':
            f = create_cmu_lines()
        elif f == 'cmupoints':
            f = create_cmu_points()
        elif f == 'miscmapinfo':
            f = create_misc_map_info()
        elif f == 'miscellaneousmapinformation':
            f = create_misc_map_info()
        elif f == 'miscmapinformation':
            f = create_misc_map_info()
        elif f == 'miscellaneousmapinfo':
            f = create_misc_map_info()
        elif f == 'layerlist':
            f = create_layer_list()
        elif f == 'standardlithology':
            f = create_standard_lithology()
        elif f == 'fossilpoints':
            f = create_fossil_points()
        elif f == 'photopoints':
            f = create_photo_points()
