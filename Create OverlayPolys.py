...
class OverlaypolysAddFields(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterFeatureSink('Overlaypolys', 'OverlayPolys', type=QgsProcessing.TypeVectorAnyGeometry, createByDefault=True, supportsAppend=True, defaultValue=None))
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        alg_params = {
            'FIELD_LENGTH': 50,
            'FIELD_NAME': 'OverlayPolys_ID',
            'FIELD_PRECISION': 0,
            'FIELD_TYPE': 2,  # Text (string)
            'INPUT': outputs['Notes']['OUTPUT'],
            'OUTPUT': parameters['Overlaypolys']
        }
        outputs['Overlaypolys_id'] = processing.run('native:addfieldtoattributestable', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        results['Overlaypolys'] = outputs['Overlaypolys_id']['OUTPUT']
        return results

    def name(self):
        return 'OverlayPolys Add Fields'
